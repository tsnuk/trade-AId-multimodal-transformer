...
- **Parameters**:
  - `num_bins` (int): Number of groups for positive values (total = 2×num_bins + 1 including zero)
...
- **⚠️ Important**: Should NOT be used on time data, categorical data, or data where exact values have semantic meaning
...
1. **Directional Success Rate**: Percentage of correct direction predictions
   - **Calculation**: `Total Wins / (Total Wins + Total Losses) × 100`
...
⚠️ **Note**: The generation functionality exists in the codebase but is **not yet integrated** into the main training workflow.
